...
Il controller HPA valuta le metriche ogni **15 secondi** (configurabile con `--horizontal-pod-autoscaler-sync-period`).

#### HPA v2 — Configurazione Completa
...
spec:
  ...
  # Scala su metrica esterna (es. lunghezza coda)
  - type: External
    external:
      metric:
      ...
      target:
        type: AverageValue
        averageValue: "30"     # 30 messaggi per Pod
  behavior:
    scaleUp:
...
#### Tipi di Metriche per HPA

| Tipo | Sorgente | Esempio | API |
|------|----------|---------|-----|
| `Resource` | metrics-server | CPU, memoria | `metrics.k8s.io` |
| `Pods` | Custom metrics adapter | req/sec, connessioni attive | `custom.metrics.k8s.io` |
| `Object` | Custom metrics adapter | dimensione coda Ingress | `custom.metrics.k8s.io` |
| `External` | External metrics adapter | messaggi in coda cloud | `external.metrics.k8s.io` |

**Prometheus Adapter** è il modo più comune per esporre metriche custom:
...
#### Behavior: Controllare Velocità di Scaling

Il campo `behavior` (introdotto in v2) permette di evitare:
- **Flapping** (scale up/down continuo): con `stabilizationWindowSeconds`
- **Scale-up troppo aggressivo**: limitando la percentuale di growth
- **Scale-down troppo veloce**: rimuovendo un Pod alla volta

```yaml
behavior:
  scaleUp:
    stabilizationWindowSeconds: 0     # Scala immediatamente (default: 0)
    selectPolicy: Max                  # Usa la policy più aggressiva
    policies:
    - type: Percent
      value: 100                       # Raddoppia
      periodSeconds: 15
    - type: Pods
      value: 4                         # Oppure aggiungi 4 Pod
      periodSeconds: 15
  scaleDown:
    stabilizationWindowSeconds: 300   # Aspetta 5 min (default: 300)
    selectPolicy: Min                  # Usa la policy meno aggressiva
    policies:
    - type: Pods
      value: 1
      periodSeconds: 60
...
**KEDA** (Kubernetes Event-Driven Autoscaling) estende HPA con 60+ sorgenti di eventi:

```yaml
# ScaledObject KEDA: scala su lunghezza coda RabbitMQ
...
spec:
  scaleTargetRef:
    name: order-processor
  minReplicaCount: 0    # ← può scalare a ZERO (HPA nativo non può!)
  maxReplicaCount: 50
  triggers:
  - type: rabbitmq
    metadata:
      queueName: orders
      queueLength: "5"   # 1 Pod per ogni 5 messaggi in coda
```

KEDA supporta: RabbitMQ, Kafka, AWS SQS, Azure Queue, Redis, PostgreSQL, Prometheus, cron, e molti altri.
...
#### VPA vs HPA: Quando Usare Quale

| | HPA | VPA |
|---|-----|-----|
| **Scala cosa** | Numero di repliche (orizzontale) | CPU/memory requests del Pod (verticale) |
| **Quando usare** | Workload stateless, scalabili orizzontalmente | Pod singoli, database, workload non scalabili |
| **Compatibilità** | ⚠️ Non usare HPA + VPA sullo **stesso** target con le stesse metriche | |
| **Scale to zero** | No (minReplicas ≥ 1) | N/A |
| **KEDA** | Sì (minReplicaCount: 0) | N/A |
...
# Conditions:
#   AbleToScale: True
#   ScalingActive: True    ← se False: metriche non disponibili!
#   ScalingLimited: False  ← se True: a min o max repliche

# Problemi comuni:
# 1. "unable to get metrics" → metrics-server non installato/non pronto
# 2. "missing request for cpu" → container senza resources.requests
# 3. ScalingActive: False → adapter custom non configurato
# 4. Target: <unknown>/60% → Pod non ha ancora metriche (appena creato)

# Vedi le metriche correnti
kubectl get hpa myapp-hpa -o yaml | grep -A 20 currentMetrics

# Simula carico per testare l'HPA
